...
class App extends React.Component {
  ...
  addGrocery(groceryItem) {
    console.log('event has made it back up!!!', groceryItem);
    let itemInList = false;
    const newList = this.state.list.slice();
    newList.forEach((listItem) => {
      if (listItem.description === groceryItem) {
        listItem.quantity++;
        itemInList = true;
      } 
    })
    if (!itemInList) {
      newList.push({id: newList.length + 1, quantity: 1, description: groceryItem })
    }
    this.setState({
      list: newList
    })
